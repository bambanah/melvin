...
**Session**
One dated, single-composition, continuous span of active-with-client work captured in the Log - the pre-Activity unit. Carries a date, a start and end time (an Open Session has a start but no end yet), the Client or group it was delivered to, and any in-session Activity Based Transport (trip km and costs) logged during it. Start and end are active-with-client times only; Provider Travel is never counted inside them. A Session later promotes into one Activity per participant. Sessions never cross midnight; one still Open when its day ends closes automatically at 23:59 (overnights are unsupported), and the Log nudges the Provider to confirm or fix that guessed end time, which stays editable like any capture.

**Handover**
The transition that closes one Session and opens the next at the same instant, keeping billable time contiguous. A **Travel Handover** is a move between locations (finish one Client, drive to the next) and captures the Inter-Client Transit distance driven; an **In-Place Handover** is a composition change at the same spot - a participant joins or leaves (flipping solo↔group), or the next Client takes over where the last one finished - and captures no driving. Composition never changes within a Session - only across a Handover.
