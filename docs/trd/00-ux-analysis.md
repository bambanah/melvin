...
The quick-activity-entry work (already shipped) got the _form_ right. What still loses to the note:
...
Sequencing assumption (per product owner): security/correctness plans `plans/001-006` land **before** all of this. TRD-002 additionally builds on plan 006 (single travel-rate source) and plan 004 (8pm boundary); TRD-003 must reconcile the group-form WIP in the working tree before starting.
