...
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------- |
| [001](./trd-001-capture-first-activity-entry.md)           | Capture-first activity entry (drafts, offline outbox, PWA, complete quick form, billing receipt)                                    | P1       | #422-#427                                      |
| [002](./trd-002-support-item-catalogue-and-rate-engine.md) | Support item catalogue & rate engine (official catalogue as data, effective-dated limits, public holidays, custom-price validation) | P1       | #425-#427                                      |
...
| [004](./trd-004-provider-travel-and-trips.md)              | Provider travel & trips UX (bulkAdd transit gap, remembered client-pair legs, travel-time completeness, one trip editor)            | P2       | #426-#427; pairs with findings #8, #12         |
...
- **Baseline**: the six security/correctness plans #422-#427 (PDF auth, secret rotation, ownership scoping, 8pm boundary, characterization tests, travel-rate single source) land **before** TRD work begins.
...
```
#422-#427 ──► TRD-001 ──► TRD-005
              └─► TRD-002 ──► TRD-003 ──► TRD-007
                          └─► TRD-004      TRD-006 (anytime after #422-#427)
```
...
- **ADR-0003** (modifiable invoiced trips) was superseded by TRD-006's sealing model — resolved: TRD-006 landed as #437, and `docs/adr/0003-modifiable-invoiced-trips.md` now carries `Status: Superseded by ADR-0004`.
...
- #434's transit-module proposal is _how_ to build TRD-002/004's engine seams — it complements, not conflicts.
- `ndis-support-catalogue.json` (refreshed to 2026-27 by #441, the interim fix) and the stale `price-guide-24-25.pdf` reference are deleted by TRD-002.
