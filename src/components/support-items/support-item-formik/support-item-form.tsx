...
import Select from "@components/forms/select-formik/select";
			...
			if (initialValues?.id) {
				...
				createSupportItemMutation
					.mutateAsync({
						supportItem: {
							...values,
							weekdayRate: Number.parseFloat(values.weekdayRate),
							weeknightRate: Number.parseFloat(values.weeknightRate),
							saturdayRate: Number.parseFloat(values.saturdayRate),
							sundayRate: Number.parseFloat(values.sundayRate),
						},
					})
					.then(() => {
